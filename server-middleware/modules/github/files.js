...
async function getFileContent (path, ref, format = 'raw') {
  ...
  try {
    const { data: file } = await github(`GET /repos/nyko28/France-PAC/contents${encodeURIComponent(path)}?ref=${ref}`, {
      path,
      mediaType: {
        format
      }
    ...
    const { data: file } = await github(`GET /repos/nyko28/France-PAC/contents${encodeURIComponent(path.replace('.md', ''))}?ref=${ref}`, {
      path,
      mediaType: {
        format
      }
...
async function getFiles (path, ref, fetchContent = false) {
  let { data: repo } = await github(`GET /repos/nyko28/France-PAC/contents${encodeURIComponent(path)}?ref=${ref}`, {
    path
  })
