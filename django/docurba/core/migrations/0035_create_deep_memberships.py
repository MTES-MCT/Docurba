# Generated by Django 6.0.6 on 2026-06-26 12:00
from django.contrib.postgres import fields, functions
from django.db import migrations, models


def create_view() -> str:
    return """
        CREATE MATERIALIZED VIEW IF NOT EXISTS materialized_view_flat_memberships AS
            WITH RECURSIVE
                adhesion_paths AS (
                    SELECT
                        from_collectivite_id,
                        to_collectivite_id,
                        1 as level,
                        ARRAY[from_collectivite_id, to_collectivite_id] AS PATH
                    FROM
                        core_collectivite_adhesions
                    UNION
                    SELECT
                        adhesion_paths.from_collectivite_id,
                        core_collectivite_adhesions.to_collectivite_id,
                        adhesion_paths.level + 1,
                        adhesion_paths.path || core_collectivite_adhesions.to_collectivite_id
                    FROM
                        adhesion_paths
                        JOIN core_collectivite_adhesions ON core_collectivite_adhesions.from_collectivite_id = adhesion_paths.to_collectivite_id
                )
        SELECT
            gen_random_uuid() as id,
            from_collectivite_id as member_id,
            to_collectivite_id as group_id,
            level,
            path
        FROM
            adhesion_paths
        group by
            member_id, group_id, level, path
        ;
    """


def create_indexes() -> str:
    return """
        CREATE INDEX flat_memberships_member_id_idx ON materialized_view_flat_memberships (member_id varchar_pattern_ops) include (group_id);

        CREATE INDEX flat_memberships_group_id_idx ON materialized_view_flat_memberships (group_id varchar_pattern_ops) include (member_id);
    """


def drop_indexes() -> str:
    return """
        DROP INDEX flat_memberships_member_id_idx;
        DROP INDEX flat_memberships_group_id_idx;
    """


class Migration(migrations.Migration):
    dependencies = [
        ("core", "0034_alter_event_date_evenement_alter_event_profile"),
    ]

    operations = [
        migrations.SeparateDatabaseAndState(
            database_operations=[
                migrations.RunSQL(
                    sql=create_view(),
                    reverse_sql="DROP MATERIALIZED VIEW IF EXISTS materialized_view_flat_memberships",
                ),
                migrations.RunSQL(
                    sql=create_indexes(),
                    reverse_sql=drop_indexes(),
                ),
                # migrations.RunSQL(
                #     sql="CREATE UNIQUE INDEX ON materialized_view_flat_memberships (id)",
                #     reverse_sql="DROP INDEX ON materialized_view_flat_memberships (id)",
                # ),
            ],
            state_operations=[
                migrations.CreateModel(
                    name="MaterializedViewFlatMembership",
                    fields=[
                        (
                            "id",
                            models.UUIDField(
                                db_default=functions.RandomUUID(),
                                primary_key=True,
                                serialize=False,
                            ),
                        ),
                        (
                            "member",
                            models.ForeignKey(
                                on_delete=models.DO_NOTHING,
                                related_name="flat_member_through",
                                to="core.Collectivite",
                                verbose_name="Membre",
                            ),
                        ),
                        (
                            "group",
                            models.ForeignKey(
                                on_delete=models.DO_NOTHING,
                                related_name="flat_group_through",
                                to="core.Collectivite",
                                verbose_name="Groupement",
                            ),
                        ),
                        (
                            "level",
                            models.SmallIntegerField(verbose_name="Niveau"),
                        ),
                        (
                            "path",
                            fields.ArrayField(
                                base_field=models.CharField(),
                                verbose_name="Chemin vers la racine",
                            ),
                        ),
                    ],
                    options={
                        "db_table": "materialized_view_flat_memberships",
                        "managed": False,
                    },
                ),
                migrations.AddField(
                    model_name="collectivite",
                    name="flat_groups",
                    field=models.ManyToManyField(
                        related_name="flat_members",
                        through="core.MaterializedViewFlatMembership",
                        to="core.collectivite",
                    ),
                ),
            ],
        )
    ]
