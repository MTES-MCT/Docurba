...
rclone_last_backup="$(rclone lsf --files-only --max-age 48h docurba:/docurba-backups | head -n 1)"
rclone copy --max-age 24h "docurba:/docurba-backups/${rclone_last_backup}" "${BACKUPS_FOLDER_PATH}"
...
if [[ ${BACKUPS_SUPABASE_MODIFY_TRIGGERS} == "True" ]]; then
  ...
  if [[ ! -n ${NUXT3_API_URL} ]]; then
    echo "Impossible de modifier les déclencheurs car NUXT3_API_URL n'est pas définie."
  fi
